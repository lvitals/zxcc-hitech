...
static int do_esc(char c)
{
	...
	if (esc1 == 27)
		switch (c) /* 1st char of escape code */
		{
		case '(':
			core_setattr(AT_BRIGHT);
			return 0;
		case ')':
			core_clearattr(AT_BRIGHT);
			return 0;
		...
		case 'o':
			...
			return 1;
		case 'p':
			core_setattr(AT_REVERSE);
			return 0;
		case 'q':
			core_clearattr(AT_REVERSE);
			return 0;
		case 'r':
			core_setattr(AT_UNDERSCORE);
			return 0;
		case 's':
			core_setattr(AT_BLINK);
			return 0;
		case 't':
			core_clearattr(AT_BLINK);
			return 0;
		case 'u':
			core_clearattr(AT_UNDERSCORE);
			return 0;
		case 'w':
			core_wrap = 1;
			return 0;
	...
	else if (escm) /* 2nd, 3rd char etc. of escape code */
	{
		...
		{
		case 'b':
			core_setfg(vt52_colour(escode[0]));
			return 1;
		case 'c':
			core_setbg(vt52_colour(escode[0]));
			return 1;
		case 'X':
			vt52_win(escode);
			return 1;
		case 'Y':
			core_move(escode[1] - 32, escode[0] - 32);
			return 1;
		}
	}
	core_addch(c);
