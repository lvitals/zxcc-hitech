...
void core_move(int x, int y)
{
	if (x < core_wx)
		x = core_wx;
	if (x >= core_wx + core_ww)
		x = core_wx + core_ww - 1;
	if (y < core_wy)
		y = core_wy;
	if (y >= core_wy + core_wh)
		y = core_wy + core_wh - 1;
...
void core_clrtoeol(void)
{
	...
	for (l = core_x; l < (core_wx + core_ww); l++)
		addch(' ');
...
void core_clrtoeos(void)
{
	...
	core_clrtoeol();
	for (l = core_y + 1; l < (core_wy + core_wh); l++)
		core_clrline(l);
}
...
/* Cursor control not currently implemented
 * type: 0=none, 1=block, 2=underline */
void core_cursor(int type)
{
	(void)type; // Explicitly mark as unused to suppress warning
...
int core_term(int func, int param)
{
	...
	case CPM_TERM_INIT:
		core_init();
		return 1;
	case CPM_TERM_GETATTR:
		return core_attr;
	case CPM_TERM_SETATTR:
		core_setattr(param);
		core_clearattr(~param);
		return 1;

	case CPM_TERM_GETCOLOUR:
		return (core_bg << 4) | core_fg;
	case CPM_TERM_SETCOLOUR:
		core_setbg((param >> 4) & 0x0F);
	...
	case CPM_TERM_CLEAR:
		...
		return 1;
	case CPM_TERM_MOVE:
		core_move(CPM_LO(param), CPM_HI(param));
		return 1;
	case CPM_TERM_IFCOLOUR:
		return has_colors();
	case CPM_TERM_HEIGHT:
		if (param > 0 && param < LINES)
			core_win(0, core_wx,
					 core_ww, param);
		return core_wh;
	case CPM_TERM_WIDTH:
		if (param > 0 && param < COLS)
			core_win(core_wy, 0,
					 param, core_wh);
		return core_ww;
